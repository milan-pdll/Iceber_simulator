...
let s2Snap = s2Meta.snapshots.find(s => s['sequence-number'] === 2)!;
let s2ManifestList = state.manifestLists[s2Snap['manifest-list']];
...
let s3Snap = s3Meta.snapshots.find(s => s['sequence-number'] === 3)!;
...
console.assert(s4Meta.snapshots.length === 5, 'Must have 5 snapshots (S0, S1, S2, S3, S4)');
...
let snapIdsToExpire = [s1Snap['snapshot-id'], s2Snap['snapshot-id']];
